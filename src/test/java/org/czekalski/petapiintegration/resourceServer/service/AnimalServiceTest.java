...
import org.czekalski.petapiintegration.resourceServer.apiResource.v1.mapper.PaginationToPaginationResourceDto;
...
class AnimalServiceTest {
    ...
    void setUp() {
        ...
        animalService = new AnimalServiceImpl(animalDtoAnimal, paginationDtoToPagination, animalToAnimalResourceDto, paginationToPaginationResourceDto, oAuth2RestTemplate, URL);
    }
    ...
    void findDogsByCityIdAndDogsQuantity() {
        ...
        given(oAuth2RestTemplate.getForObject(URL + "&location=" + CITY_ID + ", " + STATE_ID + "&limit=" + SIZE, AnimalsListDto.class))
                .willReturn(new AnimalsListDto(Arrays.asList(animalDto, animalDto2), paginationDto));

        //when
        AnimalResourcesListDto animalResourcesListDto = animalService.findDogsByCityIdAndDogsQuantity(CITY_ID, STATE_ID, SIZE);

        //then
        then(oAuth2RestTemplate).should().getForObject(URL + "&location=" + CITY_ID + ", " + STATE_ID + "&limit=" + SIZE, AnimalsListDto.class);
